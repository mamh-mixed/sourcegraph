...
func orderSearcherJob(j job.Job) job.Job {
	...
	collector := Mapper{
		MapJob: func(current job.Job) job.Job {
			if pager, ok := current.(*repoPagerJob); ok {
				if _, ok := pager.child.(*searcher.SearcherJob); ok {
	...
	orderer := Mapper{
		MapJob: func(current job.Job) job.Job {
			if pager, ok := current.(*repoPagerJob); ok {
				if _, ok := pager.child.(*zoekt.ZoektRepoSubsetSearchJob); ok && !seenZoektRepoSearch {
					seenZoektRepoSearch = true
					return NewSequentialJob(current, pagedSearcherJob)
				}
			}
			if _, ok := current.(*zoekt.ZoektGlobalSearchJob); ok && !seenZoektGlobalSearch {
