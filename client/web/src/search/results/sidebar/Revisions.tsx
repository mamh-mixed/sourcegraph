...
import { Button, LoadingSpinner, Tab, TabList, TabPanel, TabPanels, Tabs, Typography } from '@sourcegraph/wildcard'
...
const RevisionList: React.FunctionComponent<React.PropsWithChildren<RevisionListProps>> = ({
    ...
    if (error || !connection || connection.error) {
        return (
            <Typography.Text className={classNames('text-muted', styles.sidebarSectionNoResults)}>
    ...
    return (
        <>
            ...
            {(connection.totalCount ?? 0) > DEFAULT_FIRST ? (
                <Typography.Text className={classNames('text-muted d-flex', styles.sidebarSectionFooter)}>
