...
CREATE TABLE batch_spec_workspaces (
    ...
    CONSTRAINT batch_spec_workspaces_steps_check CHECK ((jsonb_typeof(steps) = 'array'::text))
);
...
COMMENT ON COLUMN lsif_indexes.indexer_args IS 'The command run inside the indexer image to produce the index file (e.g. [''lsif-typescript'', ''index''])';
